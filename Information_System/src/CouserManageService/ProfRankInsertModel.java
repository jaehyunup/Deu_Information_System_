...
public class ProfRankInsertModel  implements CourseModel{
	...
	ProfRankInsertModel(loginSpec spec) {
		this.spec = spec;
	}
	...
	public Object[][] getStuList(String coursenum){
		...
		for (int i = 0; i < this.myStdList.size(); i++) { // 검색된 강좌의 개수 만큼의행을 생성
			...
			System.out.println(CourseStuRow[i][2]);

			System.out.println(CourseStuRow[i][3]);
